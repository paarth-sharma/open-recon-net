...
def assignmentTopo():
    ...
    # s1-eth1 for h1 and s1-eth2 for h2
    net.addLink(h1, s1)
    net.addLink(h2, s1)
    
    #connect the switches
    net.addLink(s1, s2) #s1-eth3 and s2-eth3

    #s2-eth1 for h3 and s2-eth1 for h4
    net.addLink(h3, s2)
    net.addLink(h4, s2)
    ...
    # Q0 with min rate 20000000 and max rate -SELECT THE PROPER RATE-, 
    # Q1 with min rate 50000000 and max rate -SELECT THE PROPER RATE-
    os.system("sudo ovs-vsctl set port s1-eth3 qos=@newqos1 -- "
    ...
    # =>fill in here command to add one queue to the correct port of the correct switch: 
    # Q0 with min rate 20000000 and max rate -SELECT THE PROPER RATE-
    os.system("sudo ovs-vsctl set port s2-eth2 qos=@newqos2 -- "
    ...
    os.system("sudo ovs-vsctl set port s2-eth1 qos=@newqos3 -- "
